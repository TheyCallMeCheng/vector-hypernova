...
export class LoveLetterRoom extends Room<LoveLetterState> {
    ...
    startRound() {
        ...
        CARDS_DATA.forEach(c => {
            for (let i = 0; i < c.count; i++) {
                deck.push(new Card(c.value, c.name, c.desc));
            }
        });
    ...
    executeCardEffect(client: Client, card: Card, targetId?: string, guessValue?: number) {
        const player = this.state.players.get(client.sessionId);
        ...
        // 5. Prince: Discard hand
            ...
            if (!target && targetId === client.sessionId) target = player;
